//
//  DeletableCellListStackView.swift
//  DSKit
//
...
public extension Reactive where Base: DeletableCellListStackView {
...
public final class DeletableCellListStackView: View {
    ...
    public let deleteButtonTapSubject = PublishSubject<String>()
    ...
    private func createTagLabel(withText text: String) -> PaddingLabel {
        ...
        label.backgroundColor = .teamOne.mainlightColor // 태그의 배경색 설정
        ...
        let deleteButton = UIButton().then {
            $0.setButton(image: .delete4)
        }
        
        label.addSubview(deleteButton)

        deleteButton.snp.makeConstraints {
            $0.centerY.equalTo(label.snp.top).inset(5)
            $0.centerX.equalTo(label.snp.trailing).inset(5)
        }
